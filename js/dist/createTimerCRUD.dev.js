...
document.addEventListener('DOMContentLoaded', function () {
  ...
  var addInterval = function addInterval() {
    ...
    var intervalDuration = durationInput.value; // On calcul l'id en fonction du nombre d'interval dans la liste

    intervalsCompteur = document.getElementsByClassName('interval').length; // On créé un objet Interval avec id
  ...
  var refreshList = function refreshList(list, index) {
    ...
    intervalsCollection = document.getElementsByClassName('interval'); // console.log(intervalsCollection);

    if (intervalsCollection.length) {
      // On récupère la durée des intervals à lire
      for (var i = 0; i < intervalsCollection.length; i++) {
        var item = intervalsCollection.item(i).querySelectorAll('.timer__intervals__interval__item')[index].value; // console.log('item' + item);
  ...
  var returnSum = function returnSum(listOfNumber) {
    ...
    for (var i = 0; i < listOfNumber.length; i++) {
      // console.log(parseInt(listOfNumber[i]));
  ...
  var refreshTotalTime = function refreshTotalTime(durationList) {
    ...
    totalTimePreview.innerText = newTotalTimeString(sumTimes); // console.log('Appel à refreshTotalTime, nb sec : ' + sumTimes);
  };
  /**
  ...
  var pauseCountdown = function pauseCountdown(chrono) {
    if (isReading) {
      ...
      isReading = false; // console.log('Pause, temps récup : ' + stopTime);
    ...
    return new Promise(function (resolve) {
      ...
      if (isReading) {
        timerPromise = setTimeout(function () {
          isReading = false;
          resolve(true); // console.log('La promesse est true');
  ...
  /**
   * On affiche le temps total au chargement de la page puis
   * à chaque nouvel event "build"
   **/


  if (timelineName.value != '' && timelineName.value != 'Vide') {
    durationList = refreshList(durationList, 1);
