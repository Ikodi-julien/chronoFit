...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const addInterval = () => {
    ...
    // On calcul l'id en fonction du nombre d'interval dans la liste
    intervalsCompteur = document.getElementsByClassName('interval')
      .length;
  ...
  const refreshList = (list, index) => {
    ...
    if (intervalsCollection.length) {
      ...
      for (let i = 0; i < intervalsCollection.length; i++) {
        let item = intervalsCollection
          .item(i)
          .querySelectorAll('.timer__intervals__interval__item')[index].value;
